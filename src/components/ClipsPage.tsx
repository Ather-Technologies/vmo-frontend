...
import { Clip, ClipsPageProps } from '../lib/types'
import { useCookies } from 'react-cookie'
import apiFetch from "../lib/apiFetch";

function ClipsPage(props: ClipsPageProps) {
    ...
    const [cookies, setCookie] = useCookies(['clipKey', 'dateKey']);

    const onClick = useCallback((clipKey: number) => {
        // Remove old highlight
        const oldRow = document.getElementById("vmo-clip-" + props?.clipKey?.toString());
        ...
        if (clipKey) {
            ...
            props.setClipKey(clipKey);
        }
    }, [props, setCookie]);

    useEffect(() => {
        // If clipKey or dateKey is not set, terminate early
        if (!cookies?.clipKey || !cookies?.dateKey) {
            setIsLoading(false);
            return;
        }
        ...
        apiFetch(`/api/clips/${cookies.dateKey ?? 1}`)
            ...
            .then(data => {
                ...
                if (cookies.clipKey >= clips[0]?.id && cookies.clipKey <= clips[0]?.id + clips.length - 1)
                    onClick(cookies.clipKey);
                //else // Set the clipKey to the oldest clip in the list
                //  onClick(clips[0]?.id + clips.length - 1);
            });

    }, [cookies.dateKey, cookies.clipKey, onClick]);


    useEffect(() => { /// TODO------------------------------------------------------------------------------------------------------------------- CHECK DATE KEY TO SEE IF IT IS TODAY OTHERWISE DON'T PLAY ANYMORE OKEY??
        let interval: any;
        if (!cookies?.dateKey) // No sense in looking if there is no date selected
            return;

        // If the clipKey doesn't exist in the list of clips
        if (isNaN(props.clipKey) || !clips.some(clip => clip.id === props.clipKey)) {
                ...
                apiFetch(`/api/clips/${cookies.dateKey ?? 1}`)
                    ...
                    .then(data => {
                        ...
                        if (clips?.length === _clips?.length) {
                            setIsLoading(false);
                            console.log(props.clipKey, _clips)
                            return;
                        } else {
                            // Get that clippy guy
        ...
        if (currentItems.length > 0 && !currentItems.some(clip => clip.id === props.clipKey)) {
            ...
            const clipKeyIndex = clips.findIndex(clip => clip.id === props.clipKey);
        ...
        return () => clearInterval(interval);
    }, [props.clipKey, clips, currentItems, cookies.dateKey, onClick]);

    // Add highlight if on load the cookie is set
    useEffect(() => {
        // Highlight that gentleman
        const row = document.getElementById("vmo-clip-" + props?.clipKey?.toString());
        row?.classList.add("bg-slate-100");
        row?.classList.add("dark:bg-slate-700");
    }, [props.clipKey, currentItems]);
